...
const nlscLiDAR2024 = L.tileLayer(
  'https://wmts.nlsc.gov.tw/wmts/LiDAR2024/default/EPSG:3857/{z}/{y}/{x}',
  {
    attribution: '© <strong><a href="https://maps.nlsc.gov.tw/">NLSC</a> </strong>',
    minZoom: 17, //native zoom 9-20, set 17 as default 
    maxZoom: 21,
    maxNativeZoom: 20,
    bounds: ([[22.298150, 120.082789197], [23.399420713, 120.859300]]),
    crossOrigin: 'anonymous',
  });
// nlscLiDAR2024.addTo(map);

var nlscLiDAR = L.layerGroup([nlscLiDAR2019, nlscLiDAR2020,nlscLiDAR2021,nlscLiDAR2022,nlscLiDAR2023,nlscLiDAR2024],
  {
      attribution: '© <strong><a href="https://maps.nlsc.gov.tw/">NLSC</a> </strong>',
      minZoom: 17, //native zoom 9-20, set 17 as default 
      maxZoom: 21,
...
const dtmTW = L.tileLayer.colorPicker("https://raw.githubusercontent.com/wiwari/accTW/2ebf55488dd2e445d495df02e5fbf746b742e77a/dist/dem/{z}/{x}/{y}.png", {  //GITHUB exact commit
...
// const catchment = L.tileLayer.colorPicker("https://cdn.jsdelivr.net/gh/wiwari/accTW@2ebf55488dd2e445d495df02e5fbf746b742e77a/dist/acc/{z}/{x}/{y}.png", {  //CDN exact commit
// const catchment = L.tileLayer.colorPicker("https://raw.githubusercontent.com/wiwari/accTW/tiles/dist/{z}/{x}/{y}.png", {  //GITHUB
const catchment = L.tileLayer.colorPicker("https://raw.githubusercontent.com/wiwari/accTW/2ebf55488dd2e445d495df02e5fbf746b742e77a/dist/acc/{z}/{x}/{y}.png", {  //GITHUB exact commit
...
const wraRES = L.geoJSON([], {
  ...
  if (layer.feature.properties.InflowTotal){
    popupStationContainer.appendChild(document.createTextNode("進水" + (layer.feature.properties.InflowTotal * 10000 / 24 / 60 / 60).toFixed(2)));
    const subElement=L.DomUtil.create('sub','',popupStationContainer);
    subElement.appendChild(document.createTextNode("m³/s"));
    L.DomUtil.create('br','',popupStationContainer);
  }
  if (layer.feature.properties.OutflowTotal){
    popupStationContainer.appendChild(document.createTextNode("出水" + (layer.feature.properties.OutflowTotal * 10000 / 24 / 60 / 60).toFixed(2)));
    const subElement=L.DomUtil.create('sub','',popupStationContainer);
    subElement.appendChild(document.createTextNode("m³/s"));
    L.DomUtil.create('br','',popupStationContainer);
  }
...
function getwraRESstaAPI() {
      ...
      wraRESdailyAPI.forEach(rt_sta => {
        wraRESstaAPI.forEach(sta => {
          if (rt_sta.reservoiridentifier == sta['StationNo']) {
            sta['date'] = rt_sta['datetime'];
            if (rt_sta['inflow'])
              sta['InflowTotal'] = rt_sta['inflow'];
...
function getwraRESdailyAPI() {
  fetch("https://opendata.wra.gov.tw/api/v2/51023e88-4c76-4dbc-bbb9-470da690d539", {
    // TODO: https://data.wra.gov.tw/openapi/swagger
  ...
  .then((response) => {
    ...
    .then(data => {
      ...
      wraRESdailyAPI.forEach(rt_sta => {
        1;
        console.log(rt_sta.reservoiridentifier);
      });
...
// var streams = L.tileLayer.gl2({
  ...
  // tileUrls: ['https://raw.githubusercontent.com/wiwari/accTW/2ebf55488dd2e445d495df02e5fbf746b742e77a/dist/acc/{z}/{x}/{y}.png'],
  uniforms: {
