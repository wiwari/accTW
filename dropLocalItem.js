...
// Enable online GPX layer loading by drag and drop on map
...
function dropHandler(ev) {
    ...
    if (ev.dataTransfer.dropEffect == "copy" || true) {
        ...
        if (ev.dataTransfer.items) {
            // Load from dropped gpx URL
            if (url = ev.dataTransfer.getData('text/uri-list') ){
                loadOnlineGPX(url);
            }else if (url = ev.dataTransfer.getData('text/x-moz-url')){
                loadOnlineGPX(url);
            }else if (url = ev.dataTransfer.getData('public.url')){
                loadOnlineGPX(url);
            }else if (url = ev.dataTransfer.getData('text/plain')){
                loadOnlineGPX(url);
            }
        } 
    }    
...
function loadOnlineGPX(url) { //load from INPUT file or drop
    const regxFilename = /^(.*)\.[gG][pP][xX]$/;
    if (typeof (url) === 'string' && url.length > 0) //URL papameter format: ?center=lat,lng
        if (found = url.match(regxFilename)) {
            loadGPX(url);
        } else {
            console.log(`file type not supported: ${url}`); //not GPX file
            ;
        }
    ...
    if (file.name.length > 0) //URL papameter format: ?center=lat,lng
        if (found = file.name.match(regxFilename)) {
            ...
            reader.addEventListener('load', (event) => {
                ...
                gpx = event.target.result; // URL to your GPX file or the GPX itself
                loadGPX(gpx);
            });
